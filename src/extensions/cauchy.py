...
from cpp_cauchy import cauchy_mult_fwd, cauchy_mult_bwd, cauchy_mult_sym_fwd, cauchy_mult_sym_bwd

def cauchy_mult_tf(v: tf.Tensor, z: tf.Tensor, w: tf.Tensor,
                      symmetric=True) -> tf.Tensor:
    """
    v: (B, N)
    z: (L)
    w: (B, N)
    ...
    """
    if not symmetric:
        return tf.reduce_sum(rearrange(v, 'b n -> b 1 n') / (rearrange(z, 'l -> l 1') - rearrange(w, 'b n -> b 1 n')), axis=-1)
    else:
        N = v.shape[-1]
        assert N % 2 == 0
        ...
        return 2 * tf.reduce_sum((zz * vv.real - vv.real * ww.real - vv.imag * ww.imag)
                    / (zz * zz - 2 * zz * ww.real + ww.abs().square()), axis=-1)
...
def cauchy_mult(v, z, w, symmetric=True):
    ...
    N = v.shape[-1]
    assert w.shape[-1] == N
    y = _cauchy_mult(tf.reshape(v, [-1, N]), z, tf.reshape(w, [-1, N]), symmetric=symmetric)
    y = y.view(*shape[:-1], z.size(-1))
    # y = z.new_zeros(*shape[:-1], z.size(-1))
    return y


class CauchyMultiply(tf.Module, ABC):
