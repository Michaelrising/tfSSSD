...
def train(output_directory,
    ...
    train_log_dir = log_directory + stock +'/SSSD-' + alg + '/' + current_time + '_seq_{}_T_{}_Layers_{}'.format(seq_len,diffusion_config['T'], model_config['num_res_layers'])
    # train_summary_writer = tf.summary.create_file_writer(train_log_dir)

    ############## Save config file #######
    config_filename = train_log_dir + '/config_SSSD_stocks_seq_{}_T_{}_Layers_{}.json'.format(seq_len,diffusion_config['T'], model_config['num_res_layers'])
    ...
    if masking == 'rm':
        ...
        mask = get_mask_holiday(training_data) # N L C
        observed_mask = (~np.isnan(training_data)).astype(float)
        observed_mask = tf.cast(tf.transpose(observed_mask, perm=[1, 2, 0]), tf.float32)

    training_data = tf.convert_to_tensor(np.nan_to_num(training_data), dtype=tf.float32) # L N C
    print("missing rate:" +str(1-np.sum(mask)/(N*K*L)))
    ...
    loss_mask = observed_mask - mask  # .bool()
    ...
    optimizer = keras.optimizers.Adam(learning_rate=learning_rate, epsilon=1e-6, amsgrad=True)
