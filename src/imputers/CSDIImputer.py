...
class CDSIImputer(keras.Model):
    ...
    @tf.function
    def compute_loss(self, observed_data, cond_mask, target_mask, observed_tp, is_train=True, set_t=-1):
        t = tf.cond(
    ...
    def train_step(self, batch):
        observed_data, cond_mask, loss_mask = batch[0]
        ...
        with tf.GradientTape() as tape:
            loss = self.loss_fn(observed_data, cond_mask, loss_mask, observed_tp)
        learnable_params = (
    ...
    def test_step(self, batch):
        ...
        def body(t):
            loss = self.loss_fn(observed_data, cond_mask, loss_mask, observed_tp, is_train=False, set_t=t)
