...
def train(output_directory,
    ...
    while n_iter < n_iters + 1:
        for batch in training_data:
            ...
            mask = tf.tile(tf.expand_dims(mask, 0), [batch.shape[0], 1, 1]) #.float().to(device)
            loss_mask = tf.logical_not(tf.cast(mask, dtype=tf.bool))#.bool()
            batch = tf.transpose(batch, perm=[0, 2, 1]) # batch dim = [B, C, L]
            ...
            # optimizer.zero_grad()
            X = tf.cast(batch, dtype=tf.float32), tf.cast(batch, dtype=tf.float32), tf.cast(mask, dtype=tf.float32),loss_mask
            ...
            if n_iter > 0 and n_iter % iters_per_ckpt == 0:
                # tf.keras.models.save_model(net, os.path.join(output_directory, checkpoint_name))
                net.save(os.path.join(output_directory))
